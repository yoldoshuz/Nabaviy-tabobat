 ...
 * `useTranslations("catalog.<slug>")` / `blog.articles.<slug>`, so this overlay
 * is the only way live copy reaches a component — no component knows the
 * backend exists, and an unreachable backend simply leaves the bundled copy in
 * place.
 *
 ...
 * every key the components read — including the nested `features` / `benefits`
 * / `cards` groups — and keeps up with the bundle on its own. Without it
 * `getTranslations` throws MISSING_MESSAGE and the page answers 500.
 */
...
/**
 * Builds `{ catalog: {...}, blog: { articles: {...} } }` for one locale.
 *
 ...
 * copy for — otherwise a blanked `cards` group would wipe out a shipped
 * article.
 *
...
export async function buildContentMessages(
  ...
  const bundledProducts = asDict(base.catalog);
  const bundledArticles = asDict(asDict(base.blog).articles);
  const productTemplate = templateOf(bundledProducts);
  const articleTemplate = templateOf(bundledArticles);
  ...
  for (const post of posts ?? []) {
    ...
    const shipped = asDict(bundledArticles[post.slug]);
    ...
    if (Object.keys(entry).length) articleCopy[post.slug] = entry;
  }

  const overlay: Dict = {};
  if (Object.keys(productCopy).length) overlay.catalog = productCopy;
  if (Object.keys(articleCopy).length) overlay.blog = { articles: articleCopy };
