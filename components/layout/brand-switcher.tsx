    ...
    >
      ...
      */}
      <button
        ...
        className={cn(
          "group -mx-2 flex cursor-pointer items-center gap-2.5 rounded-xl px-2 py-1.5 transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold sm:gap-3",
          open ? "bg-gold/15" : "hover:bg-gold/10",
        )}
      >
        <LogoMark />
          ...
          {SIBLING_SITES.map((site) =>
            site.href ? (
              ...
              <div
                ...
                role="menuitem"
                aria-disabled
                className="flex cursor-default items-center gap-3 rounded-md p-2.5"
              >
                <BrandMark logo={site.logo} muted />
                <span className="flex-1 text-sm font-medium tracking-wide text-cream/60">
                  {site.label}
                </span>
                <span className="rounded-full bg-gold/20 px-2 py-0.5 text-[0.6875rem] font-medium text-gold">
                  {t("common.comingSoon")}
                </span>
              </div>
            ),
  ...
  return (
    ...
    >
      {Array.from({ length: 9 }, (_, index) => (
        <span key={index} className="size-[3.5px] rounded-full bg-gold" />
      ))}
    </span>
  );
}
